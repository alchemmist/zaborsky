        ...
        <div className="flex items-center gap-2 md:gap-4">
          <nav className="hidden items-center gap-7 lg:flex">
            ...
            <div className="group relative">
              ...
              <div className="invisible absolute right-0 top-full mt-3 w-80 translate-y-1 overflow-hidden rounded-2xl border border-white/20 bg-[#5c7887]/95 p-2 opacity-0 shadow-2xl shadow-black/30 backdrop-blur-xl transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
                {fenceItems.map((item) => (
                  <div key={item.label} className="rounded-xl">
                    <Link
                      href={item.href}
                      className="flex items-center justify-between rounded-xl px-4 py-3 text-left text-sm tracking-[0.14em] text-white/85 transition hover:bg-white/10 hover:text-white"
                    >
                      ...
                      <div className="space-y-1 pb-2 pl-4 pt-1">
                        {item.children.map((child) => (
                          <Link
                            key={child.label}
                            href={child.href}
                            className="block rounded-lg px-4 py-2 text-xs tracking-[0.12em] text-white/70 transition hover:bg-white/8 hover:text-white"
                          >
                            {child.label}
                          </Link>
                        ))}
